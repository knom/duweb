...
test('shows and clears postloading indicator while background prefetch runs', async ({ page }) => {
  await mockNoSuggestions(page)

  let resolveFirstLevel: (() => void) | null = null
  ...
  const postloadingIndicator = page.locator('span.border-cyan-200', { hasText: 'Loading...' })
  await expect(postloadingIndicator).toBeVisible()
  resolveFirstLevel?.()
  await expect(postloadingIndicator).toHaveCount(0)
})
